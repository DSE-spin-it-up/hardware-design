...
elevator_deflection_up = np.radians(25)     # deg
elevator_deflection_down = np.radians(20)   # deg
...
taue=(CLh-CLalphah*alphah)/(CLalphah*elevator_deflection_up) # CLh tail lift coefficient, CLalphah is horizontal tail lift curve slope, alphah is horizontal tail angle of attack in radians, elevator_deflection_up is elevator deflection in radians
...
def cE_ch(taue):
    ...
    # Valid elevator cbord ratio range
    valid = roots[(roots >= 0.2) & (roots <= 0.4)]

    if len(valid) == 0:
        raise ValueError(f"No valid root found for taue={taue}")
...
CLdeltaE = CLalphah * etah * Vh * Sh/S * bE_bh * taue # CLalphah is horizontal tail lift curve slope, etah is dynamic pressure ratio at the horizontal tail, Vh is horizontal tail volume coefficient, bE_bh is elevator span to horizontal tail span ratio, Sh_S is horizontal tail area to wing area ratio
