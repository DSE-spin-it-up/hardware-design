...
Tail rod torsion: the side force generated by maximum rudder deflection
acts above the boom centerline, so the tail rod is checked for combined
bending + torsion using the von Mises criterion. If the torsion check
governs, the diameter is increased accordingly.
...
class RodResult:
    ...
    tail_boom_torsion_force: float = 0.0   # rudder side force used for boom torsion [N]
    tail_boom_torsion_torque: float = 0.0  # torque used for boom torsion [N*m]
...
def apply_torsion_check(
    ...
    return dataclasses.replace(
        ...
        tail_boom_torsion_force = abs(float(side_force)),
        tail_boom_torsion_torque = abs(float(torque)),
    )


def rudder_side_force_torque(
    sizing: SizingResult,
    rudder,
    q: float,
    CLalphav_vt: float,
    ...
    """Return rudder side force and boom torque at max rudder deflection.

    The force is the rudder-control contribution only. It acts at half the
    vertical-tail span per the current tail-boom torsion sizing assumption.
    """
    bR_bV = rudder.geometry.bR_bV
    side_force = q * rudder.geometry.Sv * CLalphav_vt * eta_v * (
        rudder.tau_r
        * rudder.delta_R
        * rudder.inputs.cR_cV
        * bR_bV ** 2
    )
    torque = side_force * 0.5 * rudder.geometry.bv
    return float(side_force), float(torque)
...
def run(
    ...
    if rudder is not None and CLalphav_vt != 0.0:
        ...
        # ---- rudder rod ----
        # Same bending load model as spar rod. Hinge moment is intentionally
        # not used for rod sizing at this stage.
